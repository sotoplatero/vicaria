<script lang="ts">
	...
	function renderMarkdown(md: string): string {
		return md
			.replace(
				/^### (.*$)/gim,
				'<h3 class="text-3xl font-semibold mb-4 mt-8">$1</h3>',
			)
			.replace(
				/^## (.*$)/gim,
				'<h2 class="text-4xl font-light mb-6 mt-12">$1</h2>',
			)
			.replace(
				/^# (.*$)/gim,
				'<h1 class="text-5xl font-light mb-8">$1</h1>',
			)
...
<!-- Hero Section -->
<section
	class="min-h-[500px] bg-gradient-to-br from-primary/10 to-secondary/10"
>
	<div class="container mx-auto px-4 py-20">
		<div class="max-w-4xl mx-auto text-center">
			...
			<p
				class="text-xl md:text-2xl text-base-content/80 mb-8 leading-relaxed"
			>
				{service.description}
			</p>
			<a
				href="tel:+13653369757"
				class="btn btn-primary btn-lg hover:scale-105 transition-transform"
			>
...
<section class="py-20 bg-primary text-primary-content">
	<div class="container mx-auto px-4 text-center max-w-3xl">
		...
		<div class="flex flex-col sm:flex-row gap-4 justify-center">
			<a
				href="tel:+13653369757"
				class="btn btn-secondary btn-lg hover:scale-105 transition-transform"
			>
				<Calendar class="w-5 h-5" />
				Book Appointment
			</a>
			<a
				href="/#contact"
				class="btn btn-outline btn-secondary btn-lg hover:scale-105 transition-transform"
			>
				Contact Us
			</a>
		</div>
