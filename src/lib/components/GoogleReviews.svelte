<script lang="ts">
	...
	let { reviews = [], rating = 0, totalReviews = 0, error = '' }: Props = $props();
	...
	<div class="container mx-auto px-4 max-w-6xl">
			...
			{#if !error && rating > 0}
				<div class="flex items-center justify-center gap-2 text-xl">
					...
					<div class="flex">
						{#each Array(5) as _, i}
							<Star
								class="w-6 h-6 {i < Math.floor(rating) ? 'fill-warning text-warning' : 'text-base-300'}"
							/>
						{/each}
					</div>
					<span class="text-base-content/70">({totalReviews} reviews)</span>
				</div>
			{/if}
		</div>
		...
		{#if error}
			<div class="alert alert-warning max-w-2xl mx-auto">
				<span>Unable to load reviews at this time. Please check back later.</span>
			</div>
							...
							<div class="flex items-start gap-3 mb-3">
								...
								{:else}
									<div class="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center">
										<span class="font-bold">{review.author_name[0]}</span>
									</div>
								{/if}
								<div class="flex-1">
									...
									<div class="flex gap-1">
										{#each Array(5) as _, i}
											<Star
												class="w-4 h-4 {i < review.rating ? 'fill-warning text-warning' : 'text-base-300'}"
											/>
										{/each}
									</div>
									<p class="text-sm text-base-content/70">{formatDate(review.time)}</p>
								</div>
							</div>
